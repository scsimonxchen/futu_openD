...
public class FTAPI_Conn_Trd extends FTAPI_Conn {
    ...
    /***
     * 获取组合的可买卖信息，具体字段请参考Trd_GetComboMaxTrdQtys.proto协议
     * @param req
     * @return 请求的序列号
     */
    public int getComboMaxTrdQtys(TrdGetComboMaxTrdQtys.Request req) {
        return sendProto(ProtoID.TRD_GETCOMBOMAXTRDQTYS, req);
    }
...
//        }
    ...
    /***
     * 组合期权下单，具体字段请参考Trd_PlaceComboOrder.proto协议
     * @param req
     * @return 请求的序列号
     */
    public int placeComboOrder(TrdPlaceComboOrder.Request req) {
        return sendProto(ProtoID.TRD_PLACECOMBOORDER, req);
    }
...
//        }
     ...
     * @return 请求的序列号
     */
    public int getFlowSummary(TrdFlowSummary.Request req) {
        return sendProto(ProtoID.TRD_GETFLOWSUMMARY, req);
    }
    ...
    protected void onReply(ReqReplyType replyType, ProtoHeader protoHeader, byte[] data) {
        ...
        switch (protoID) {
            ...
            case ProtoID.TRD_GETCOMBOMAXTRDQTYS: {
                TrdGetComboMaxTrdQtys.Response rsp;
                if (replyType == ReqReplyType.SvrReply) {
                    try {
                        rsp = TrdGetComboMaxTrdQtys.Response.parseFrom(data);
                    } catch (InvalidProtocolBufferException e) {
                        rsp = TrdGetComboMaxTrdQtys.Response.newBuilder().setRetType(Common.RetType.RetType_Invalid_VALUE).build();
                    }
                } else {
                    rsp = TrdGetComboMaxTrdQtys.Response.newBuilder().setRetType(replyType.getCode()).build();
                }

                trdSpi.onReply_GetComboMaxTrdQtys(this, serialNo, rsp);
            }
            break;
            ...
            case ProtoID.TRD_PLACECOMBOORDER: {
                TrdPlaceComboOrder.Response rsp;
                if (replyType == ReqReplyType.SvrReply) {
                    try {
                        rsp = TrdPlaceComboOrder.Response.parseFrom(data);
                    } catch (InvalidProtocolBufferException e) {
                        rsp = TrdPlaceComboOrder.Response.newBuilder().setRetType(Common.RetType.RetType_Invalid_VALUE).build();
                    }
                } else {
                    rsp = TrdPlaceComboOrder.Response.newBuilder().setRetType(replyType.getCode()).build();
                }

                trdSpi.onReply_PlaceComboOrder(this, serialNo, rsp);
            }
            break;
            ...
            break;
            case ProtoID.TRD_GETFLOWSUMMARY: {
                TrdFlowSummary.Response rsp;
                if (replyType == ReqReplyType.SvrReply) {
                    try {
                        rsp = TrdFlowSummary.Response.parseFrom(data);
                    } catch (InvalidProtocolBufferException e) {
                        rsp = TrdFlowSummary.Response.newBuilder().setRetType(Common.RetType.RetType_Invalid_VALUE).build();
                    }
                } else {
                    rsp = TrdFlowSummary.Response.newBuilder().setRetType(replyType.getCode()).build();
                }

                trdSpi.onReply_GetFlowSummary(this, serialNo, rsp);
            }
            break;
        }
    }
