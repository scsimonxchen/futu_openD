    ...
    long findAccNumber(TrdGetAccList.S2C s2c, TrdCommon.TrdEnv trdEnv, TrdCommon.TrdMarket trdMarket,
        ...
        for (int i = 0; i < nLength; i++) {
            if (s2c.getAccList(i).getTrdEnv() == trdEnv.getNumber()) {
                boolean isSupportTrd = false;
                for (int j = 0; j < s2c.getAccList(i).getTrdMarketAuthListCount(); j++) {
                    if (s2c.getAccList(i).getTrdMarketAuthList(j) == trdMarket.getNumber()) {
                        isSupportTrd = true;
                        break;
                    }
                }
                if (!isSupportTrd) continue;
