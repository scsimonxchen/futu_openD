...
public interface DataStore extends Closeable {
    ...
    void saveQuoteArchive(String apiName, String entityKey, GeneratedMessageV3 response);
    ...
    static DataStore open(Path dbPath, OutputFormat format, AppConfig config) {
        ...
        if (format == OutputFormat.MYSQL) {
            if (config == null || config.getMySqlConfig() == null) {
                throw new IllegalStateException("MySQL format requires mysql.* settings in config.properties");
            }
            return new MySqlStore(config.getMySqlConfig());
        }
